...
const en = {
  ...
  biggerContextBody: "Warning: keeps small turns as one message, then splits larger context across two, three, or six messages. The final part starts the task without an extra request. It triples the model context and compaction thresholds; restart Codex after changing it. Extra requests may increase rate limits or temporary cooldowns. Disabled by default.",
  contextLength: "Context length",
  contextLengthBody: "Default keeps this ChatGPT Web route's current budget. Larger profiles upload adaptively in one, two, three, or at most six messages; short continuations remain one message. Restart Codex after changing it.",
  contextDefault: "Default",
  manualContextLengthUnavailable: "Context profiles are available only with Automation. Zero Risk keeps its fixed manual profile.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  contextLength: "上下文长度",
  contextLengthBody: "默认会保留该 ChatGPT Web 路由当前的上下文预算。较大的 Profile 会按需分成 1、2、3 或最多 6 段上传；短续轮仍只发送一段。修改后请重启 Codex。",
  contextDefault: "默认",
  manualContextLengthUnavailable: "上下文 Profile 仅适用于自动模式。零风险模式继续使用固定的手动 Profile。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小さなターンは 1 件のメッセージとして維持し、大きなコンテキストは 2、3、または 6 件のメッセージに分割します。最後の部分から追加リクエストなしでタスクを開始します。モデルのコンテキスト上限とコンパクションしきい値が 3 倍になります。変更後は Codex を再起動してください。追加リクエストにより、レート制限や一時的なクールダウンが発生しやすくなる場合があります。初期設定では無効です。",
  contextLength: "コンテキスト長",
  contextLengthBody: "Default はこの ChatGPT Web ルートの現在の予算を維持します。大きいプロファイルは 1、2、3、最大 6 件のメッセージへ適応的に分割し、短い継続は 1 件のままです。変更後は Codex を再起動してください。",
  contextDefault: "Default",
  manualContextLengthUnavailable: "コンテキスト Profile は Automation でのみ利用できます。Zero Risk は固定の手動 Profile を維持します。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "경고: 작은 턴은 하나의 메시지로 유지하고, 큰 컨텍스트는 두 개, 세 개 또는 여섯 개의 메시지로 나눕니다. 마지막 부분에서 별도의 추가 요청 없이 작업을 시작합니다. 모델 컨텍스트와 컴팩션 임계값이 3배로 증가합니다. 변경 후 Codex를 다시 시작하세요. 추가 요청으로 인해 사용량 제한이나 일시적인 쿨다운이 더 자주 발생할 수 있습니다. 기본적으로 비활성화되어 있습니다.",
  contextLength: "컨텍스트 길이",
  contextLengthBody: "Default는 이 ChatGPT Web 경로의 현재 예산을 유지합니다. 더 큰 프로필은 1개, 2개, 3개 또는 최대 6개의 메시지로 적응형 전송하며 짧은 연속 턴은 1개로 유지합니다. 변경 후 Codex를 다시 시작하세요.",
  contextDefault: "Default",
  manualContextLengthUnavailable: "컨텍스트 Profile은 Automation에서만 사용할 수 있습니다. Zero Risk는 고정된 수동 Profile을 유지합니다.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小型回合仍以一則訊息傳送；大型上下文會拆成兩則、三則或六則訊息。最後一部分會直接開始任務，不會額外傳送請求。模型上下文與壓縮門檻會提高至三倍；變更後請重新啟動 Codex。額外請求可能更容易觸發速率限制或暫時冷卻。預設關閉。",
  contextLength: "上下文長度",
  contextLengthBody: "預設會保留此 ChatGPT Web 路由目前的上下文預算。較大的 Profile 會按需分成 1、2、3 或最多 6 段上傳；短續輪仍只傳送一段。變更後請重新啟動 Codex。",
  contextDefault: "預設",
  manualContextLengthUnavailable: "上下文 Profile 僅適用於自動模式。零風險模式會保留固定的手動 Profile。",
