...
test("long-running browser observations do not spin at sub-250ms cadence", () => {
  expect(CHATGPT_BROWSER_OBSERVATION_POLL_MS).toBe(500);
  expect(CHATGPT_MULTIPART_ACKNOWLEDGEMENT_POLL_MS).toBe(1_000);

  const workerSource = readFileSync(new URL("../src/adapters/chatgpt-web/browser-worker.ts", import.meta.url), "utf8");
  const multipartLoop = workerSource.slice(
  ...
  expect(multipartLoop).toContain("CHATGPT_MULTIPART_ACKNOWLEDGEMENT_POLL_MS");
  expect(multipartLoop).not.toContain("setTimeout(resolveSleep => setTimeout(resolveSleep, 100)");
